    ...
    if(project['status']==200): #check if the project exists
        ...
        for i in tags['data']:
            if(i['name']==tag):
                tag_id=i['pk']
                break
        data={'project_tags':list(set(project['data']['project_tags']+[tag_id]))}   #update the tags of the project
        my_account.Projects.update_project_details(id, data)
        print("The projects with the tag "+tag+" are:") #show the projects with the tag
        for i in tags['data']:
            if(i['pk']==tag_id):
