...
def show_tasks_for_projects():
    ...
    res=my_account.Tasks.create_task({'priority':str(2),'title':'Test Task','project':str(id_project),'due_date':'30-04-2022','assigned_to':str(pk_employee),'labels':[],'description':"",'list':"",'phase':""}) #add a task to a random project
    ...
    for i in my_account.Tasks.get_tasks_list()['data']:
        if(not i['is_completed'] and i['project']==id_project):
            tasks.append((i['due_date'],i['priority_display'],i['title'],i['assigned_to_display'],i['project_display']))

    print("Tasks for the project {0}:".format(tasks[0][4])) #list the tasks of the previous project
    for i in tasks:
