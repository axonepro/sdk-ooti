...
class Expenses(Helper):
    ...
    def get_expenses_categories_list(self, page=1):
        ...
        route = "v1/expenses/category/list/{0}/?page_size={1}&page={2}".format(
    ...
    def add_multiple_expenses(self, expense_group_id, files):
        ...
        route = "v1/expenses/groups/create-multiple-expenses/{0}/".format(
            expense_group_id
        )
        response = self.process_request(
            requests, "POST", self.base_url, route, self.headers, None, files
        )
        return self.process_response(response)
    ...
    def get_expenses_groups_list(self, team_pk=None, page=1):
        ...
        if team_pk is None:
            route = "v1/expenses/groups/v2/list/{0}/?page_size={1}&page={2}".format(
                self.org_pk, self.pagination, page
            )
        else:
            route = "v1/expenses/groups/list/{0}/?page_size={1}&page={2}".format(
    ...
    def get_expenses_list(self, team_pk=None, expense_group_id=None, page=1):
        ...
        if team_pk is not None and expense_group_id is not None:
            ...
            route = "v1/expenses/my-groups/list/{0}/?page_size={1}&page={2}".format(
                team_pk, self.pagination, page
            )
        else:
            route = "v1/expenses/list/{0}/?page_size={1}&page={2}".format(
                self.org_pk, self.pagination, page
            )
        response = self.process_request(
            requests, "GET", self.base_url, route, self.headers, None, None
